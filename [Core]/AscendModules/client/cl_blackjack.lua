...
RMenu.Add('CLIMBblackjack', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('CLIMBblackjack', 'instructions'):SetSubtitle("~d~BLACKJACK")
RMenu.Add('CLIMBblackjack_high', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('CLIMBblackjack_high', 'instructions'):SetSubtitle("~d~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CLIMBblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('CLIMBblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CLIMBblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('CLIMBblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('CLIMBblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('CLIMBblackjack_high', 'instructions'), flag)
    end
end
