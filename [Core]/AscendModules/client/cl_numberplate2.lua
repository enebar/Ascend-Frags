...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.ButtonWithStyle("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("CLIMB:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('CLIMB:getCars')
...
RegisterNetEvent("CLIMB:RecieveNumberPlate")
AddEventHandler("CLIMB:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("CLIMB:carsTable")
AddEventHandler("CLIMB:carsTable",function(cars)
    carstable = cars
end)
