RMenu.Add('CLIMBPoliceMenu', 'main', RageUI.CreateMenu(""," Police",GetRageUIMenuWidth(),GetRageUIMenuHeight(), "banners", "police"))
RMenu.Add('CLIMBPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("CLIMBPoliceMenu", "main")))
RMenu:Get('CLIMBPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('CLIMBPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CLIMBPoliceMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                RageUI.ButtonWithStyle("Object Menu" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) end, RMenu:Get('CLIMBPoliceMenu', 'objectmenu'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CLIMBPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CLIMBPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.ButtonWithStyle("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('CLIMB:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('CLIMB:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("CLIMB:PoliceMenuOpened")
AddEventHandler("CLIMB:PoliceMenuOpened",function()
  RageUI.Visible(RMenu:Get('CLIMBPoliceMenu', 'main'), not RageUI.Visible(RMenu:Get('CLIMBPoliceMenu', 'main')))
end)
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('CLIMB:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('CLIMB:ZoneCreated')
AddEventHandler('CLIMB:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('CLIMB:RemovedBlip')
AddEventHandler('CLIMB:RemovedBlip', function()
...
RegisterNetEvent("CLIMB:DragPlayer")
AddEventHandler('CLIMB:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
	...
	while true do 
		if isInJail == true then 
			if isInArea(v5, 20.4) then 
				...
				CLIMB.notify({"~d~You have been teleported back to prison!"})
