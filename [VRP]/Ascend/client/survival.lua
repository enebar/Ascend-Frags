  ...
  while true do 
    if IsDisabledControlJustPressed(0,38) then
      if playerCanRespawn and in_coma and secondsTilBleedout < 2 then
        ...
        tCLIMB.respawnPlayer()
        DoScreenFadeIn(900)
        TriggerEvent("CLIMB:StartRespawnCam")
        TriggerEvent("CLIMB:OpenRespawnMenu")
      end
      Wait(1000)
    end
    Wait(0)
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            CLIMBserver.Coma()
            TriggerEvent('CLIMB:IsInMoneyComa', true)
            TriggerServerEvent('CLIMB:InComa')
            CLIMBserver.MoneyDrop()
            TriggerEvent('CLIMB:5Seconds')
            tCLIMB.ejectVehicle()
            in_coma = true
            TriggerEvent("CLIMB:PlaySound", tCLIMB.getDeathSound())
...
RegisterNetEvent("CLIMB:5Seconds")
AddEventHandler("CLIMB:5Seconds", function()
    DeathAnim = 5
    while DeathAnim <= 5 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
RegisterNetEvent("CLIMB:FixClient")
AddEventHandler("CLIMB:FixClient", function()
...
AddEventHandler("CLIMB:respawnButtonClicked",function(X, Y)
    if Y and Y > 0 then
        TriggerServerEvent("CLIMB:takeAmount", Y)
    end
    local Z = p.spawnLocations[X].coords
    TriggerEvent("CLIMB:playGTAIntro")
    tCLIMB.ClearWeapons()
    if tCLIMB.isHandcuffed() then
        TriggerEvent("CLIMB:toggleHandcuffs", false)
    end
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            if DeathReason == 'committed suicide' or DeathReason == 'died' then
                ...
                local Id = TriggerServerCallback("CLIMB:FetchKillerID", GetPlayerServerId(Killer))
                if Id then
