...
RMenu.Add('CLIMBBackpacks', 'main', RageUI.CreateMenu("", "Backpack Store",1300, 50, "banners", "garages"))
RMenu.Add('CLIMBBackpacks', 'submenu', RageUI.CreateMenu("", "Backpack Purchase",1300, 50, "banners", "garages"))
RageUI.CreateWhile(1.0, RMenu:Get('CLIMBBackpacks', 'main'), nil, function()
    RageUI.IsVisible(RMenu:Get('CLIMBBackpacks', 'main'), true, false, true, function() 
        for k,v in pairs() do 
            RageUI.ButtonWithStyle(v[1], v[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            end, RMenu:Get('CLIMBBackpacks', 'submenu'))
        end
    end)
end)
...
RegisterNetEvent("CLIMB:getBackPacks")
AddEventHandler("CLIMB:getBackPacks", function(a)
    if a ~= nil then
        backpacks = a
    end
