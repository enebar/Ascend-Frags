-- basic CLIMB food.lua with tacos which give health.
...
local function gen(ftype, health)
    ...
    local name = CLIMB.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = CLIMB.getUserId(player)
      if user_id ~= nil then
        if CLIMB.tryGetInventoryItem(user_id,idname,1,false) then
          ...
          if ftype == "drink" then
            CLIMBclient.notify(player,{"~d~Drinking "..name.."."})
            play_drink(player)
          elseif ftype == "eat" then
            CLIMBclient.notify(player,{"~o~Eating "..name.."."})
            play_eat(player)
          end
