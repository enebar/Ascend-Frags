...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('CLIMB:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('CLIMB:purgeRespawn')
    end
end)
...
RegisterNetEvent("CLIMB:activatePurge")
AddEventHandler("CLIMB:activatePurge", function()
    purgeActivatingAnnounce = true
    TriggerEvent("CLIMB:PlaySound", "purge")
    Wait(10000)
