--DiamondBlackjack created by Robbster. CLIMB Edit by DocWeird.


local Tunnel = module('CLIMB', 'lib/Tunnel')
local Proxy = module('CLIMB', 'lib/Proxy')
CLIMB = Proxy.getInterface("CLIMB")
CLIMBclient = Tunnel.getInterface("CLIMB", "Blackjack")
...
function tryTakeChips(source,amount)
    ...
    if CLIMB.tryPayment({user_id,amount}) then 
        return true
    end
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if CLIMB.hasGroup({userid, 'HighRoller'}) then
        if chairId > 7 then
            if source ~= nil then
                for k,v in pairs(blackjackTables) do 
                    if v == source then 
...
AddEventHandler("Blackjack:requestSitAtBlackjackTableNorm", function(chairId)
    ...
    if CLIMB.hasPermission({userid, "player.phone"}) then
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent("CLIMB:PlaySound", source, "casino_lose")
                                                    local command = {
                                                        {
                                                            ["color"] = "15806495",
                                                            ["title"] = " Casino Logs",
                                                            ...
                                                            ["fields"] = {
                                                                ...
                                                                {
                                                                    ["name"] = "Player Bank",
                                                                    ["value"] = "£"..parseInt(tonumber(CLIMB.getBankMoney({CLIMB.getUserId({source})}))),
                                                                    ["inline"] = true
                                                                },
                                                                {
                                                                    ["name"] = "Player Cash",
                                                                    ["value"] = "£"..parseInt(tonumber(CLIMB.getMoney({CLIMB.getUserId({source})}))),
                                                                    ["inline"] = true
                                                                }
                                                            }
                                                        }
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("CLIMB:PlaySound", source, "casino_win")
                                                            local command = {
                                                                {
                                                                    ["color"] = "3135805",
                                                                    ["title"] = " Casino Logs",
