...
RegisterNetEvent("CLIMB:initMoney",function(n, o)
    m = o
end)
local function p(q)
...
local function r(s, t)
    ...
    while #(tCLIMB.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tCLIMB.loadModel("metrotrain")
    tCLIMB.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function G()
    while DoesEntityExist(j) and GetTrainDoorOpenRatio(j, 0) > 0.05 do
        SetCanClimbOnEntity(j, false)
        local H = tCLIMB.getPlayerCoords()
...
local function N()
    TriggerEvent("CLIMB:Announce", "FOLLOW THE YELLOW MARKERS!")
...
local function O()
    local P = tCLIMB.addMarker(b.x, b.y, b.z, 1.0, 1.0, 1.0, 50, 205, 50, 150, 50, 32, false, false, true)
    ...
    ScaleformMovieMethodAddParamTextureNameString("CLIMB TUTORIAL")
    ScaleformMovieMethodAddParamTextureNameString("~g~Welcome to CLIMB")
    ...
    local E = PlayerPedId()
    while true do
        local R = #(tCLIMB.getPlayerCoords() - b)
        if R < 3.0 then
    ...
    tCLIMB.removeMarker(P)
    SetScaleformMovieAsNoLongerNeeded(Q)
    tCLIMB.loadAnimDict("cellphone@")
    tCLIMB.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tCLIMB.getPlayerCoords()
...
local function V()
    ...
    while tCLIMB.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tCLIMB.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    local C = tCLIMB.getPlayerVehicle()
    while tCLIMB.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tCLIMB.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tCLIMB.getPlayerVehicle() == 0 do
        if RageUI.Visible(RMenu:Get("CLIMBGarages", "main")) then
            ...
            h = "Select Garages"
        elseif RageUI.Visible(RMenu:Get("CLIMBGarages", "owned_vehicles")) then
            i = "This lists all your garage types. For now you will only have the Standard Garage."
            h = "Select Standard Garage"
        elseif RageUI.Visible(RMenu:Get("CLIMBGarages", "owned_vehicles_submenu")) then
            i = "This lists all the vehicles you have bought for this garage type."
            h = "Select your newly purchased vehicle"
        elseif RageUI.Visible(RMenu:Get("CLIMBGarages", "owned_vehicles_submenu_manage")) then
            i = "You can spawn your vehicle, or choose to sell and rent it to another player here."
            h = "Press Spawn Vehicle"
        elseif RageUI.Visible(RMenu:Get("CLIMBGarages", "settings")) then
            i = nil
            h = "Go back to the main menu"
        elseif
            RageUI.Visible(RMenu:Get("CLIMBGarages", "rented_vehicles")) or RageUI.Visible(RMenu:Get("CLIMBGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("CLIMBGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tCLIMB.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tCLIMB.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a4()
...
local function a6()
    ...
    local a7 = tCLIMB.getCustomization()
    local a8 = false
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("CLIMBclothing") then
            if a8 then
                break
            end
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tCLIMB.getPlayerCoords()) < 2.5
            if a2 then
            ...
            if RageUI.Visible(RMenu:Get("CLIMBclothing", "mainMenu")) then
                ...
                h = "Select Clothes"
            elseif RageUI.Visible(RMenu:Get("CLIMBclothing", "changeClothing")) then
                i = "You can pick from an arrangement of stock and custom clothing. Scroll through to see each item."
                h = "Change your outfit"
            elseif RageUI.Visible(RMenu:Get("CLIMBclothing", "changePed")) then
            ...
            elseif RageUI.Visible(RMenu:Get("CLIMBclothing", "clearProps")) then
                i = "This can be used to remove accessories. You can also toggle them through the Y menu."
                h = "Go back to pick an outfit"
            end
            if not table.contentEquals(tCLIMB.getCustomization(), a7) then
                a8 = true
            end
        end
        Citizen.Wait(0)
...
local function ac()
    ...
    EndScaleformMovieMethod()
    p("questcomplete")
    TriggerServerEvent("CLIMB:setCompletedTutorial")
    local U = GetGameTimer()
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tCLIMB.isDevMode() then
        if m > 20000 then
            a1()
        end
    else
    ...
    ab()
    ac()
    tCLIMB.setCanAnim(true)
    g = false
end
RegisterCommand("tutorial",function()
    if tCLIMB.isDevMode() then
        ad()
    end
end,false)
RegisterNetEvent("CLIMB:playTutorial", ad)
AddEventHandler("CLIMB:onClientSpawn",function(ae, af)
    if af and not tCLIMB.isPurge() then
        Citizen.Wait(10000)
        TriggerServerEvent("CLIMB:checkTutorial")
    end
end)
local function ag()
    if g then
        ...
        if not tCLIMB.isPurge() then
            ...
            tCLIMB.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tCLIMB.createThreadOnTick(ag)
RegisterNetEvent("CLIMB:setIsNewPlayer",function()
    f = true
end)
function tCLIMB.isNewPlayer()
    return f
end
function tCLIMB.isInTutorial()
    return g
end
