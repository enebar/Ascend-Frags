...
function tCLIMB.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("CLIMB:applyModifications", name, nveh)
            TriggerServerEvent("CLIMB:PayVehicleTax")
        ...
        tCLIMB.notify("~d~Vehicle " ..vehiclename.. " is already out.")
    end
end

function tCLIMB.despawnGarageVehicle(vtype, max_range)
    local Vehicle = tCLIMB.getNearestVehicle(3)
...
function tCLIMB.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
function tCLIMB.fixeNearestVehicle(radius)
    local veh = tCLIMB.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tCLIMB.replaceNearestVehicle(radius)
    local veh = tCLIMB.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tCLIMB.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tCLIMB.getNearestOwnedVehicle(radius)
...
-- return ok,x,y,z
function tCLIMB.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tCLIMB.getOwnedVehiclePosition(vtype)
...
function tCLIMB.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tCLIMB.vc_openDoor(hash, door_index)
...
function tCLIMB.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tCLIMB.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tCLIMB.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tCLIMB.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tCLIMB.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tCLIMB.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tCLIMB.notify("~d~Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tCLIMB.notify("~g~Vehicle locked.")
        end
    end
end
