...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("CLIMB:IDsAboveHead", true)
    else
        TriggerServerEvent("CLIMB:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('CLIMB:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('CLIMB:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("CLIMB:ChangeIDs")
AddEventHandler("CLIMB:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
