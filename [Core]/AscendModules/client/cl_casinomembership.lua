...
RMenu.Add("CLIMB:CasinoLicenseStore", "main", RageUI.CreateMenu("", "~d~Casno Membership", GetRageUIMenuWidth(),GetRageUIMenuHeight(),"banners","licenses"))
RMenu.Add("CLIMB:CasinoLicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("CLIMB:CasinoLicenseStore", "main"), "", "~d~License Store", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("CLIMB:CasinoLicenseStore", "remove", RageUI.CreateSubMenu(RMenu:Get("CLIMB:CasinoLicenseStore", "main"), "", "~d~Confirm Remove License", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("CLIMB:CasinoLicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("CLIMB:CasinoLicenseStore", "sub"), "", "~d~Confirm License Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("CLIMB:CasinoLicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("CLIMB:CasinoLicenseStore", "main"), true, false, true, function()

        RageUI.ButtonWithStyle("Casino Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "casino"
            end
        end, RMenu:Get("CLIMB:CasinoLicenseStore", "sub", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("CLIMB:CasinoLicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.ButtonWithStyle(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("CLIMB:CasinoLicenseStore", "confirm"):SetSubtitle("~d~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("CLIMB:CasinoLicenseStore", "confirm", true))
            end
        end
        ...
        end, RMenu:Get("CLIMB:CasinoLicenseStore", "remove", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("CLIMB:CasinoLicenseStore", "confirm"), true, false, true, function()
        ...
        end, RMenu:Get("CLIMB:CasinoLicenseStore", "main", true))
    ...
    RageUI.IsVisible(RMenu:Get("CLIMB:CasinoLicenseStore", "remove"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Removal", nil, {RightLabel = "~g~£0"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("CLIMB:RemoveLicense", currentLicense)
            end
        end, RMenu:Get("CLIMB:CasinoLicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.casinolicenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("CLIMB:CasinoLicenseStore", "main"), true)
            end
        end
