    ...
    if #(playerCoords - boxCoords) < 2.0 then
        if not used then
                ...
                if lootrandom == 1 then -- [Legendary]
                    -- [Legendary]
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_MOSIN', 1, true})
                    CLIMB.giveInventoryItem({user_id, '7.62 Bullets', 250, true})

                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_SPAR16', 1, true})
                    CLIMB.giveInventoryItem({user_id, '5.56 NATO', 250, true})
                    CLIMBclient.notify(source,{'Received ~g~£400,000 Cash.'})
                    CLIMB.giveMoney({user_id,400000})
                ...
                elseif lootrandom == 2 then 
                    ...
                    CLIMBclient.notify(source,{'Received ~g~£300,000 Cash.'})
                    CLIMB.giveMoney({user_id,300000})
                    TriggerClientEvent('chat:addMessage', -1, {
                ...
                elseif lootrandom == 3 then 
                    ...
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_M1911', 1, true})
                    CLIMB.giveInventoryItem({user_id, '9mm Bullets', 50, true})
                    CLIMBclient.notify(source,{'Received ~g~£200,000 Cash.'})
...
Citizen.CreateThread(function()
    while (true) do
        ...
        if #GetPlayers() > 5 then
            ...
            TriggerClientEvent('chatMessage', -1, "^0CLIMB │ ^0", {255, 255, 255}, dropMsg, "alert")
            TriggerClientEvent("CLIMB:PlaySound", -1, "supplydrop")
        end
        used = false
    end
end)
