local m = module("CLIMBVehicles", "cfg/cfg_garages")
m=m.garage_types
...
-- No-Clip Handler
RegisterServerEvent("CLIMB:Type1")
AddEventHandler("CLIMB:Type1", function()
    local user_id = CLIMB.getUserId(source)
    local player = CLIMB.getUserSource(user_id)
    local name = GetPlayerName(source)
    if not CLIMB.hasPermission(user_id, "admin.noclip") then -- give this group to users you do want getting banned for No-Clipping
        if not table.includes(carrying, player) then
            ...
            CLIMB.setBanned(user_id, true, "perm", reason, adminPermID)
            f10Ban(user_id, adminPermID, reason, "perm")
            CLIMB.kick(player,"[CLIMB] You have been permanently banned from CLIMB.  \n\nReason: " .. reason .. "\n\nYou have been banned by: " .. adminPermID .. "\n\n [Your ID: " .. user_id .. "]") 
...
RegisterServerEvent("CLIMB:Type2") -- Player Spawned Weapon!
AddEventHandler("CLIMB:Type2", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
...
AddEventHandler("playerDropped", function(reason)
    ...
    local name = GetPlayerName(source)
    local user_id = CLIMB.getUserId(source)
    CLIMB.getUserIdByIdentifiers(GetPlayerIdentifiers(source), function(u)
...
RegisterServerEvent("CLIMB:Type5") -- Player Spawned Weapon!
AddEventHandler("CLIMB:Type5", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
...
RegisterServerEvent("CLIMB:Type6") -- Player Spawned Weapon!
AddEventHandler("CLIMB:Type6", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
...
RegisterServerEvent("CLIMB:Type7") -- Player Spawned Weapon!
AddEventHandler("CLIMB:Type7", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
...
RegisterServerEvent("CLIMB:Type8") -- Player Spawned Weapon!
AddEventHandler("CLIMB:Type8", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
...
RegisterServerEvent("CLIMB:Type9") -- Triggering Events
AddEventHandler("CLIMB:Type9", function()
	local user_id = CLIMB.getUserId(source)
	local player = CLIMB.getUserSource(user_id)
	local name = GetPlayerName(source)
