          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                CLIMBcfgdrugsServer.IronGather()
                Action = false
              else
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              CLIMBcfgdrugsServer.IronCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  CLIMBcfgdrugsServer.IronDoneProcessing()
                  CLIMB.notify({"~w~Processed~g~ Iron~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  CLIMB.notify({"~d~You do not have the required license."})
                    ...
                    if not ketAmount then
                      CLIMB.notify({"~d~You do not have enough Iron Ore."})
                    end
                end
              end)
...
function sellingIronUnits()
     CLIMBcfgdrugsServer.SellIronJob({tonumber(1)})
    return false
end
