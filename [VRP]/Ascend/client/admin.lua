    ...
    Citizen.CreateThread(function()
        ...
        while DoesBlipExist(blip) do
            if GetBlipInfoIdType(blip) == 4 then
                CLIMB, cy, cz = table.unpack(Citizen.InvokeNative(0xFA7C7F0AADF25D09, blip, Citizen.ReturnResultAnyway(), Citizen.ResultAsVector())) --GetBlipInfoIdCoord(blip)
                blipFound = true
                break
            end
        ...
        if blipFound then
            ...
            for i = 0, 1000, 1 do
                SetEntityCoordsNoOffset(entity, CLIMB, cy, ToFloat(i), false, false, false)
                SetEntityRotation(entity, 0, 0, 0, 0 ,0)
                SetEntityHeading(entity, yaw)
                SetGameplayCamRelativeHeading(0)
                Citizen.Wait(0)
                if GetGroundZFor_3dCoord(CLIMB, cy, ToFloat(i), cz, false) then --GetGroundZFor3dCoord(CLIMB, cy, i, 0, 0) GetGroundZFor_3dCoord(CLIMB, cy, i)
                    cz = ToFloat(i)
                    groundFound = true
                    break
                end
        ...
        if success then
            SetEntityCoordsNoOffset(entity, CLIMB, cy, cz, false, false, true)
            SetGameplayCamRelativeHeading(0)
...
RegisterNetEvent("CLIMB:vehicleMenu")
AddEventHandler("CLIMB:vehicleMenu",function(DioMode, isInTicket)
    if getStaffLevel() > 0 then
	OMioDioMode = DioMode
    ticketStatus = isInTicket
    TriggerEvent('greenzoneStaffMode', OMioDioMode)
	if not OMioDioMode then
		tCLIMB.setCustomization(adminTicketSavedCustomization)
        setRedzoneTimerDisabled(false)
        ...
        adminTicketSavedCustomization = tCLIMB.getCustomization()
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('CLIMB:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterCommand("fix", function()
    if OMioDioMode or tCLIMB.getUserId() == 2 or tCLIMB.getUserId() == 1 then
...
RegisterNetEvent("CLIMB:showBlips")
AddEventHandler("CLIMB:showBlips",function()
    blips = not blips
    if blips then
    ...
    while true do 
        if tCLIMB.isStaffedOn()then 
            ...
            if not tCLIMB.isInTicket()then 
                tCLIMB.drawNativeText("~d~Special Powerz Activated, Remember To Staff Off/Return!",255,0,0,255,true)
            end 
        end;
        Wait(0)
    end 
...
function tCLIMB.inCardevBucket()
    return IsInCardevBucket
end

function tCLIMB.drawNativeText(V)
