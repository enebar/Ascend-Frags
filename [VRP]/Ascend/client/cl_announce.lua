...
RegisterNetEvent("CLIMB:Announce")
AddEventHandler(
    "CLIMB:Announce",
    function(d)
        if d ~= nil then
            CreateThread(
                function()
                    ...
                    BeginScaleformMovieMethod(scaleform, "SHOW_SHARD_MIDSIZED_MESSAGE")
                    ScaleformMovieMethodAddParamTextureNameString("~y~CLIMB Announcement")
...
RegisterNetEvent(
    "CLIMB:smallAnnouncement",
    function(f, g, h, i)
        CLIMB.announceMpSmallMsg(f, g, h, i)
    end
)
function CLIMB.announceMpSmallMsg(f, g, h, i)
