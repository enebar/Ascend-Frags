...
RMenu.Add("CLIMB:LicenseStore", "main", RageUI.CreateMenu("", "~d~License Store", 1350, 50,"banners","licenses"))
RMenu.Add("CLIMB:LicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "main"), "", "~d~License Store", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "illegal", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "main"), "", "~d~License Store", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "sub"), "", "~d~Confirm License Purchase", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "illegalconfirm", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "illegal"), "", "~d~Confirm License Purchase", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "main"), "", "~d~License Store", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "refund", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "main"), "", "~d~Refund License ", 1350, 50))
RMenu.Add("CLIMB:LicenseStore", "confirmrefund", RageUI.CreateSubMenu(RMenu:Get("CLIMB:LicenseStore", "refund"), "", "~d~Confirm Refund License Purchase", 1350, 50))
RageUI.CreateWhile(1.0, RMenu:Get("CLIMB:LicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "main"), true, false, true, function()

        RageUI.ButtonWithStyle("~g~Grinding Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "grinding"
            end
        end, RMenu:Get("CLIMB:LicenseStore", "sub", true))

        RageUI.ButtonWithStyle("~d~Illegal Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "other"
            end
        end, RMenu:Get("CLIMB:LicenseStore", "illegal", true))
        
        RageUI.ButtonWithStyle("Refund Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
            TriggerServerEvent("CLIMB:RequestLicensescenter")
            end
        end, RMenu:Get("CLIMB:LicenseStore", "refund", true))
    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.ButtonWithStyle(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("CLIMB:LicenseStore", "confirm"):SetSubtitle("~d~Purchase "..v.name.."?")
                    end
                end, RMenu:Get("CLIMB:LicenseStore", "confirm", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "illegal"), true, false, true, function()
        ...
        for k, v in pairs(cfg.illegal) do
            if v.type == currentType then
                ...
                end, RMenu:Get("CLIMB:LicenseStore", "illegalconfirm", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "confirm"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Purchase", nil, {RightLabel = "~g~£"..getMoneyStringFormatted(currentLicensePrice)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("CLIMB:BuyLicense", currentLicense)
            end
        end, RMenu:Get("CLIMB:LicenseStore", "main", true))
    ...
    RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "illegalconfirm"), true, false, true, function()

        RageUI.Separator("You are buying "..currentLicense.." for ~g~£"..getMoneyStringFormatted(currentLicensePrice))
        RageUI.Separator("")
        RageUI.ButtonWithStyle("~g~Confirm Purchase", nil, {RightLabel = "~g~£"..getMoneyStringFormatted(currentLicensePrice)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("CLIMB:BuyIllegalLicense", currentLicense)
            end
        end, RMenu:Get("CLIMB:LicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.licenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("CLIMB:LicenseStore", "main"), true)
            end
        end
...
RageUI.CreateWhile(1.0, RMenu:Get("CLIMB:LicenseStore", "refund"), nil, function()
RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "refund"), true, false, true, function()
...
for k, v in pairs(cfg.refundlicenese) do
        RageUI.ButtonWithStyle(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.refund)}, true, function(Hovered, Active, Selected)
            if Selected then
                ...
                RMenu:Get("CLIMB:LicenseStore", "confirmrefund"):SetSubtitle("~d~Refund "..v.name.."?")
            end
        end, RMenu:Get("CLIMB:LicenseStore", "confirmrefund", true))
    end
end)
RageUI.IsVisible(RMenu:Get("CLIMB:LicenseStore", "confirmrefund"), true, false, true, function()
    ...
    RageUI.Separator("You will receive ~g~£"..getMoneyStringFormatted(currentLicenserefund).. "~w~ for refunding "..currentLicense)
                ...
                RageUI.ButtonWithStyle("~g~Confirm Refund", nil, {RightLabel = "~g~£"..getMoneyStringFormatted(currentLicensePrice)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent("CLIMB:RemoveLicensecenter", currentLicense)
                    end
                end, RMenu:Get("CLIMB:LicenseStore", "refund", true))
                RageUI.ButtonWithStyle("~d~Cancel", nil, {}, true, function(Hovered, Active, Selected)
                end, RMenu:Get("CLIMB:LicenseStore", "refund", true))
