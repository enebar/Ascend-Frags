...
RegisterCommand("anon", function(source, args, raw)
    ...
    if anoncooldown[source] and not (os.time() > anoncooldown[source]) then
        TriggerClientEvent('chatMessage', source, "CLIMB",  { 255, 255, 255 }, "You are being rate limited.", "alert")
        return
    else
...
RegisterCommand("p", function(source,args, rawCommand)
    user_id2 = CLIMB.getUserId(source)   
    if not CLIMB.hasPermission(user_id2, "police.armoury") then
        local playerName = "Server "
        local message = "Access denied."
        TriggerClientEvent('chatMessage', source, "^7Alert: " , { 128, 128, 128 }, message, "alert")
    end
    if CLIMB.hasPermission(user_id2, "police.armoury") then
        local message = rawCommand:sub(2)
        local playerName =  GetPlayerName(source)
        local players = GetPlayers()
        for i,v in pairs(players) do 
...
RegisterCommand("a", function(source,args, rawCommand)
    local user_id = CLIMB.getUserId(source)   
    if CLIMB.hasPermission(user_id, "chat.announce") then
        local message = rawCommand:sub(2)
        local playerName =  GetPlayerName(source)
...
function ooc(source,args,raw)
    ...
    elseif CLIMB.hasGroup(user_id, 'director') then 
...
RegisterCommand('clear', function(source, args, rawCommand)
    local user_id = CLIMB.getUserId(source)
    if CLIMB.hasPermission(user_id, 'admin.ban') then
        local players = GetPlayers()
        for i,v in pairs(players) do 
            local source = v
        ...
        CLIMBclient.notify(source,{"~d~You do not have permission to use this command."})
    end
end, false)
