...
CLIMBclient = Tunnel.getInterface("CLIMB","CLIMB_bank")
...
AddEventHandler('es_bank:rob', function(robb)
	...
	if CLIMB.hasPermission({user_id,"police.armoury"}) then
		CLIMBclient.notify(player,{"~d~Police Officers can't rob stores."})
	else
		if #cops > 2 then
			if banks[robb] then
				...
				SetTimeout(180000, function()
					if(robbers[savedSource])then
						if(user_id)then
							CLIMB.giveMoney({user_id, math.random(50000,100000)})
			...
			CLIMBclient.notify(player,{"~d~There are not enough Police online."})
		end 
	end    
end)
