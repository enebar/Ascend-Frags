globalBlips = {}
function tCLIMB.addBlip(a, b, c, d, e, f, g, h)
...
function tCLIMB.removeBlip(j)
    RemoveBlip(j)
end
function tCLIMB.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
local k = {}
function tCLIMB.setNamedBlip(l, a, b, c, d, e, f, g)
    tCLIMB.removeNamedBlip(l)
    k[l] = tCLIMB.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tCLIMB.removeNamedBlip(l)
    if k[l] ~= nil then
        tCLIMB.removeBlip(k[l])
        k[l] = nil
    end
end
function tCLIMB.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tCLIMB.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tCLIMB.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tCLIMB.removeMarker(j)
    if m[j] ~= nil then
        m[j] = nil
        n:free(j)
    end
...
function tCLIMB.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tCLIMB.removeNamedMarker(l)
    o[l] = tCLIMB.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tCLIMB.removeNamedMarker(l)
    if o[l] ~= nil then
        tCLIMB.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
Citizen.CreateThread(
    function()
        while true do
            local M = tCLIMB.getPlayerCoords()
            J = {}
...
Citizen.CreateThread(
    function()
        while true do
            q = tCLIMB.getNearbyMarkers()
            Citizen.Wait(10000)
        end
    end
)
function tCLIMB.getNearbyMarkers()
    local N = {}
    local O = tCLIMB.getPlayerCoords()
    local P = 0
...
local Q = {}
local R = {}
function tCLIMB.getNearbyAreas()
    local S = {}
    local M = tCLIMB.getPlayerCoords()
    local P = 0
    for K, L in pairs(Q) do
...
function tCLIMB.setArea(l, a, b, c, T, U)
...
RegisterCommand("nextblip",function()
    a5 = a5 + 1
    if a4 then
        tCLIMB.removeBlip(a4)
    end
    a4 = tCLIMB.addBlip(1103.9739990234, 211.95138549805, -49.440101623535, a5, 0, "Chips Cashier", 0.8, true)
end)
