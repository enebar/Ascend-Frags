...
RegisterNetEvent('CLIMB:load')
AddEventHandler('CLIMB:load', function(list)
	doors = list
end)

RegisterNetEvent('CLIMB:statusSend')
AddEventHandler('CLIMB:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("CLIMB:open", id)
				end
			end
		end
	end
