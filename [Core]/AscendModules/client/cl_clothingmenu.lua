RMenu.Add('CLIMBclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tCLIMB.getRageUIMenuWidth(),tCLIMB.getRageUIMenuHeight(),"gmt_clothesui","gmt_clothesui"))
RMenu.Add('CLIMBclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('CLIMBclothing','mainMenu'),"","Change Gender"))
RMenu.Add('CLIMBclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('CLIMBclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('CLIMBclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('CLIMBclothing','mainMenu'),"","Clear Props"))
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tCLIMB.getPlayerPed())
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('CLIMBclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('CLIMBclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
        if RageUI.Visible(RMenu:Get('CLIMBclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('Clothes',nil,{RightLabel="→→→"},true,function(L,M,N)
                    if N then 
                        D()
                    end 
                end,RMenu:Get('CLIMBclothing','changeClothing'))
                if not inOrganHeist then
                    RageUI.Button('Change Gender',nil,{RightLabel="→→→"},true,function(L,M,N)
                    end,RMenu:Get('CLIMBclothing','changePed'))
                end
                RageUI.Button('~y~Wardrobe',nil,{RightLabel="→→→"},true,function(L,M,N)
                    if N then 
                        if tCLIMB.isPlusClub()or tCLIMB.isPlatClub()then
                            TriggerEvent("CLIMB:openOutfitMenu")
                        else 
                            tCLIMB.notify("~y~You need to be a subscriber of CLIMB Plus or CLIMB Platinum to use this feature.")
                            tCLIMB.notify("~y~Available @ store.gmtstudios.uk")
                        end 
                    end 
                end)
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~CLIMB Plus~w~ or ~y~CLIMB Platinum~w~.',{RightLabel="→→→"},true,function(L,M,N)
                    if N then 
                        tCLIMB.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("CLIMB:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('CLIMBclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('CLIMBclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tCLIMB.loadCustomisationPreset("DefaultMale")
                        else
                            tCLIMB.notify("Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tCLIMB.loadCustomisationPreset("DefaultFemale")
                        else
                            tCLIMB.notify("Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('CLIMBclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("CLIMB:saveClothingHairData",GetPedDrawableVariation(tCLIMB.getPlayerPed(),2),GetPedHairColor(tCLIMB.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tCLIMB.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tCLIMB.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tCLIMB.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tCLIMB.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tCLIMB.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tCLIMB.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tCLIMB.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tCLIMB.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tCLIMB.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tCLIMB.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tCLIMB.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tCLIMB.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('CLIMBclothing','changeClothing'))then 
...
AddEventHandler("CLIMB:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tCLIMB.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tCLIMB.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
