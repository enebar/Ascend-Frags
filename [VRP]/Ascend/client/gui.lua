...
-- pause
AddEventHandler("CLIMB:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tCLIMB.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tCLIMB.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tCLIMB.prompt(title,default_text)
...
function tCLIMB.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    CLIMBserver.closeMenu({data.id})
  elseif data.act == "valid" then
    CLIMBserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    CLIMBserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    CLIMBserver.requestResult({data.id,data.ok})
  end
end)
...
function tCLIMB.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/CLIMB/issues/89)
for i=1,5 do
...
function tCLIMB.setProgressBar(name,anchor,text,r,g,b,value)
...
function tCLIMB.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tCLIMB.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tCLIMB.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tCLIMB.isInComa() or not cfg.coma_disable_menu) and (not tCLIMB.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      CLIMBserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("CLIMB:pauseChange", paused)
