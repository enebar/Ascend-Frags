RMenu.Add('CLIMBtruckjob', 'main', RageUI.CreateMenu("", "~g~Truck Job", nil, nil, "banners", ""))
RMenu:Get('CLIMBtruckjob', 'main'):SetPosition(1350, 10)
...
local truckJobPay = 0
RageUI.CreateWhile(1.0, RMenu:Get('CLIMBtruckjob', 'main'), nil, function()

    RageUI.IsVisible(RMenu:Get('CLIMBtruckjob', 'main'), true, false, true, function()
            ...
            RageUI.ButtonWithStyle(v.name , "~g~On completion you will receive £" ..Comma(v.pay), { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if not jobStarted then
                        ...
                        TriggerServerEvent('CLIMB:startTruckJob', v.name)
                    else
            ...
            RageUI.ButtonWithStyle("End Job" , "~d~Stop your current job", { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if jobStarted then
                        ...
                        TriggerServerEvent('CLIMB:finishTruckJob')
                        truckJobPay = 0
                    else
        ...
        if isInArea(vector3(truckjob.startLocation), 2.5) then 
            ...
            if IsControlJustPressed(0, 51) then 
                if not IsPedSittingInAnyVehicle(PlayerPedId()) then
                    RageUI.CloseAll()
                    RageUI.Visible(RMenu:Get("CLIMBtruckjob", "main"), true)
                else
                    ...
                    if jobStarted then
                        if (IsPedSittingInAnyVehicle(PlayerPedId())) then
                            if IsVehicleModel(GetVehiclePedIsIn(PlayerPedId(), true), GetHashKey(truckjob.vehicle)) then
                                ...
                                TriggerServerEvent('CLIMB:finishTruckJob', currentTruckJob)
                                jobStarted = false
                                truckJobCoords = nil
...
function SpawnTruck()
	...
	SetVehicleNumberPlateText(spawned_car, "CLIMB")
