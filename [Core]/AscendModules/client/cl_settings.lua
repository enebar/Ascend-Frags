RMenu.Add('SettingsMenu', 'MainMenu', RageUI.CreateMenu("", "~b~Settings Menu", tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(), "banners","settings")) 
RMenu.Add("SettingsMenu", "graphicpresets", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Graphics Presets',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "changediscord", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Link Discord',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "killeffects", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Kill Effects',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "bloodeffects", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Blood Effects',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "weaponswhitelist", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Custom Weapons Owned',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "generateaccesscode", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "weaponswhitelist"), "", '~b~Generate Access Code',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "viewwhitelisted", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "generateaccesscode"), "", '~b~View Whilelisted Users',tCLIMB.getRageUIMenuWidth(), tCLIMB.getRageUIMenuHeight(),"banners","settings"))
...
Citizen.CreateThread(function()
    local f = GetResourceKvpString("vesper_diagonalweapons") or "false"
    if f == "false" then
        b = false
        TriggerEvent("CLIMB:setVerticalWeapons")
    else
        b = true
        TriggerEvent("CLIMB:setDiagonalWeapons")
    end
    local g = GetResourceKvpString("vesper_frontars") or "false"
    if g == "false" then
        c = false
        TriggerEvent("CLIMB:setBackAR")
    else
        c = true
        TriggerEvent("CLIMB:setFrontAR")
    end
    local h = GetResourceKvpString("vesper_hitmarkersounds") or "false"
    if h == "false" then
        d = false
        TriggerEvent("CLIMB:hsSoundsOff")
    else
        d = true
        TriggerEvent("CLIMB:hsSoundsOn")
    end
    local m = GetResourceKvpString("vesper_hideeventannouncement") or "false"
    if m == "false" then
        g = false
    else
        g = true
    end
    local n = GetResourceKvpString("vesper_healthpercentage") or "false"
    if n == "false" then
        h = false
    else
        h = true
...
function tCLIMB.setDiagonalWeaponSetting(i)
    SetResourceKvp("vesper_diagonalweapons", tostring(i))
end
function tCLIMB.setFrontARSetting(i)
    SetResourceKvp("vesper_frontars", tostring(i))
end
function tCLIMB.setHitMarkerSetting(i)
    SetResourceKvp("vesper_hitmarkersounds", tostring(i))
end
function tCLIMB.setCODHitMarkerSetting(i)
    SetResourceKvp("vesper_codhitmarkersounds", tostring(i))
end
function tCLIMB.setHideEventAnnouncementFlag(p)
    SetResourceKvp("vesper_hideeventannouncement", tostring(p))
end
function tCLIMB.getHideEventAnnouncementFlag()
    return g
end
function tCLIMB.setShowHealthPercentageFlag(p)
    SetResourceKvp("vesper_healthpercentage", tostring(p))
end
function tCLIMB.getShowHealthPercentageFlag()
    return h
end
local function j(k)
    RageUI.CloseAll()
...
local r
local s
RegisterNetEvent("CLIMB:gotCustomWeaponsOwned",function(t)
    print("gotCustomWeaponsOwned", dump(t))
    o = t
end)
RegisterNetEvent("CLIMB:generatedAccessCode",function(u)
    print("got accessCode", u)
    r = u
end)
RegisterNetEvent("CLIMB:getWhitelistedUsers",function(v)
    s = v
end)
local w = {}
...
local function G(y, z, H)
    ...
    SetResourceKvp("vesper_graphic_presets", I)
end
local J = {
    "0%",
    "5%",
...
local function U()
    ...
    SetResourceKvp("vesper_kill_effects", V)
end
local W = {head = 1, body = 1, arms = 1, legs = 1}
local function X()
    local Y = json.encode(W)
    SetResourceKvp("vesper_blood_effects", Y)
end
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    local I = GetResourceKvpString("vesper_graphic_presets")
    if I and I ~= "" then
        w = json.decode(I) or {}
    end
    ...
    local V = GetResourceKvpString("vesper_kill_effects")
    if V and V ~= "" then
    ...
    local Y = GetResourceKvpString("vesper_blood_effects")
    if Y and Y ~= "" then
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'MainMenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            local function a4()
                TriggerEvent("CLIMB:setDiagonalWeapons")
                b = true
                tCLIMB.setDiagonalWeaponSetting(b)
            end
            local function a5()
                TriggerEvent("CLIMB:setVerticalWeapons")
                b = false
            ...
            local function a4()
                TriggerEvent("CLIMB:hsSoundsOn")
                d = true
                tCLIMB.setHitMarkerSetting(d)
                tCLIMB.notify("~y~Experimental Headshot sounds now set to " .. tostring(d))
            end
            local function a5()
                TriggerEvent("CLIMB:hsSoundsOff")
                d = false
            ...
            RageUI.ButtonWithStyle("Weapon Whitelists","Sell your custom weapon whitelists here.",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    ...
                    q = nil
                    s = nil
                    TriggerServerEvent("CLIMB:getCustomWeaponsOwned")
                end
            ...
            RageUI.Checkbox("Streetnames","",tCLIMB.isStreetnamesEnabled(),{RightBadge = RageUI.CheckboxStyle.Car},function(a0, a2, a1, a6)
            end,
            function()
                tCLIMB.setStreetnamesEnabled(true)
            end,
            function()
                tCLIMB.setStreetnamesEnabled(false)
            end)
            RageUI.Checkbox("Compass","",tCLIMB.isCompassEnabled(),{RightBadge = RageUI.CheckboxStyle.Car},function(a0, a2, a1, a6)
            end,
            function()
                tCLIMB.setCompassEnabled(true)
            end,
            function()
                tCLIMB.setCompassEnabled(false)
            end)
            local function a4()
                tCLIMB.hideUI()
                hideUI = true
            ...
            RageUI.Checkbox("Show Health Percentage","Displays the health and armour percentage on the bars.",h,{},function()
                ...
                h = true
                tCLIMB.setShowHealthPercentageFlag(true)
            end,function()
                h = false
                tCLIMB.setShowHealthPercentageFlag(false)
            end)
            RageUI.Checkbox("Hide Event Announcements","Hides the big scaleform from displaying across your screen, will still announce in chat.",g,{},function()
                ...
                g = true
                tCLIMB.setHideEventAnnouncementFlag(true)
            end,function()
                g = false
                tCLIMB.setHideEventAnnouncementFlag(false)
            end)
            RageUI.ButtonWithStyle("Change Linked Discord","Begins the process of changing your linked Discord. Your linked discord is used to sync roles with the server.",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent('CLIMB:changeLinkedDiscord')
                end
            end)
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'changediscord')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Enter Code","",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent('CLIMB:enterDiscordCode')
                end
            end)
        end)
    end
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'generateaccesscode')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if a2 then
                    ...
                    if tonumber(a9) then
                        ...
                        if a9 > 0 then
                            ...
                            TriggerServerEvent("CLIMB:generateWeaponAccessCode", q, a9)
                        end
                    end
                end
            end)
            RageUI.ButtonWithStyle("View whitelisted users","",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent("CLIMB:requestWhitelistedUsers", q)
                end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.List("~y~Particles~w~",P,S.particle,"",{},true,function(a0, a1, a2, a3)
                if a1 and S.particle ~= a3 then
                    if not tCLIMB.isPlusClub() and not tCLIMB.isPlatClub() then
                        notify("~y~You need to be a subscriber of CLIMB Plus or CLIMB Platinum to use this feature.")
                        notify("~y~Available @ store.vesperrp.co.uk")
                    end
                    S.particle = a3
                    U()
                end
            ...
            if GetGameTimer() - T > ad + 1000 then
                tCLIMB.addKillEffect(PlayerPedId(), true)
                T = GetGameTimer()
            end
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'bloodeffects')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.List("~y~Head",P,W.head,"Effect that displays when you hit the head.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tCLIMB.addBloodEffect("head", 0x796E, PlayerPedId())
                end
            end)
            RageUI.List("~y~Body",P,W.body,"Effect that displays when you hit the body.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tCLIMB.addBloodEffect("body", 0x0, PlayerPedId())
                end
            end)
            RageUI.List("~y~Arms",P,W.arms,"Effect that displays when you hit the arms.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tCLIMB.addBloodEffect("arms", 0xBB0, PlayerPedId())
                    tCLIMB.addBloodEffect("arms", 0x58B7, PlayerPedId())
                end
            end)
            RageUI.List("~y~Legs",P,W.legs,"Effect that displays when you hit the legs.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tCLIMB.addBloodEffect("legs", 0x3FCF, PlayerPedId())
                    tCLIMB.addBloodEffect("legs", 0xB3FE, PlayerPedId())
                end
            end)
        end)
    end
...
AddEventHandler("CLIMB:enteredCity",function()end)
AddEventHandler("CLIMB:leftCity",function()end)
...
function tCLIMB.addKillEffect(au, av)
    if S.lightning then
        ForceLightningFlash()
    end
    if S.pedFlash then
    ...
    if S.particle ~= 1 and (tCLIMB.isPlatClub() or av) then
        Citizen.CreateThreadNow(function()
            ar(au)
        end)
    end
...
function tCLIMB.addBloodEffect(aw, ax, ag)
    local ay = W[aw]
    if ay > 1 then
        local as = Q[ay]
        tCLIMB.loadPtfx(as[1])
...
AddEventHandler("CLIMB:onPlayerKilledPed",function(az)
    tCLIMB.addKillEffect(az, false)
end)
local aA = {
    [0x0] = "body",
    [0x2E28] = "body",
...
AddEventHandler("CLIMB:onPlayerDamagePed",function(az)
    if not tCLIMB.isPlusClub() and not tCLIMB.isPlatClub() then
        return
    end
    ...
    if aB then
        ...
        tCLIMB.addBloodEffect(aE, ax, az)
    end
end)

RegisterNetEvent("CLIMB:gotDiscord")
AddEventHandler("CLIMB:gotDiscord",function()
