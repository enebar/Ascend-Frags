          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                CLIMBcfgdrugsServer.WeedGather ()
                Action = false
              else
                CLIMB.notify({"~d~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              CLIMBcfgdrugsServer.CanProcessWeed({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then

          


                  CLIMBcfgdrugsServer.ProcessWeed()

                  CLIMB.notify({"~w~Processed~g~ Weed~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  CLIMB.notify({"~d~You do not have the required license."})
                    ...
                    if not ketAmount then
                      CLIMB.notify({"~d~You do not have enough Cannabis Sativa."})
                    end
                end
              end)
              ClearPedTasksImmediately(ped)
...
function sellingWeedUnits()
    CLIMBcfgdrugsServer.SellWeed({tonumber(1)})
    return false
end
