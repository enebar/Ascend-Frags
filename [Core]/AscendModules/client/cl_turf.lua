...
RegisterNetEvent('CLIMB:TakenTurf')
AddEventHandler('CLIMB:TakenTurf', function(isnTurf)
...
RegisterNetEvent('CLIMB:OutOfZone')
AddEventHandler('CLIMB:OutOfZone', function(isnTurf)
	inTurf = false
...
RegisterNetEvent('CLIMB:PlayerDied')
...
RegisterNetEvent('CLIMB:TurfComplete')
AddEventHandler('CLIMB:TurfComplete', function(reward, name)
	inTurf = false
...
Citizen.CreateThread(function()
	while true do
				...
				if not inTurf then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos3.x, pos3.y, pos3.z) < 1.4) then
						...
						if (IsControlJustReleased(1, 51)) then
							TriggerServerEvent('CLIMB:rob', k)
		...
		if inTurf then		
			...
			TriggerServerEvent('CLIMB:PlayerDied', turf)

			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > turfs[turf].radius) then
				TriggerServerEvent('CLIMB:TooFar', turf)
			end
		end
