...
RMenu.Add("CLIMB:Identity", "main", RageUI.CreateMenu("Identity Shop", "~d~Identity Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("CLIMB:Identity", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("CLIMB:Identity", "main"), true, false, true, function()
                            ...
                            if secondName ~= "" then  
                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if tonumber(age) and tonumber(age) < 100 then 
                                        TriggerServerEvent("CLIMB:ChangeIdentity", firstName, secondName, tonumber(age))
                                    else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInArea(cfg.location, 1.0) and inMenu == false then
            ...
            RageUI.Visible(RMenu:Get("CLIMB:Identity", "main"), true)
            inMenu = true
            end
        end
        ...
        if isInArea(cfg.location, 1.0) == false and inMenu then
            inMenu = false
            RageUI.Visible(RMenu:Get("CLIMB:Identity", "main"), false)
        end
    end
end)
