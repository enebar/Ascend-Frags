...
--- x,y,z: coordinates
--- msg: alert message
function CLIMB.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service then
    ...
    for k,v in pairs(CLIMB.rusers) do
      ...
      -- check user
      if CLIMB.hasPermission(k,service.alert_permission) and player ~= nil then
    ...
    for k,v in pairs(players) do
      CLIMBclient.notify(v,{service.alert_notify..msg})
      -- add position for service.time seconds
      CLIMBclient.addBlip(v,{x,y,z,service.blipid,service.blipcolor,"("..service_name..") "..msg}, function(bid)
        SetTimeout(service.alert_time*1000,function()
          CLIMBclient.removeBlip(v,{bid})
        end)
      end)

      -- call request
      if sender ~= nil then
        CLIMB.request(v,lang.phone.service.ask_call({service_name, htmlEntities.encode(msg)}), 30, function(v,ok)
          if ok then -- take the call
            if not answered then
              -- answer the call
              CLIMBclient.notify(sender,{service.answer_notify})
              CLIMBclient.setGPS(v,{x,y})
              answered = true
            else
              CLIMBclient.notify(v,{lang.phone.service.taken()})
            end
          end
        end)
      end
...
function CLIMB.sendSMS(user_id, phone, msg, cbr)
  local task = Task(cbr)
  ...
  CLIMB.getUserIdentity(user_id, function(identity)
    CLIMB.getUserByPhone(phone, function(dest_id)
      if identity and dest_id then
        local dest_src = CLIMB.getUserSource(dest_id)
        if dest_src then
          local phone_sms = CLIMB.getPhoneSMS(dest_id)

          if #phone_sms >= cfg.sms_history then -- remove last sms of the table
            table.remove(phone_sms)
          end

          local from = CLIMB.getPhoneDirectoryName(dest_id, identity.phone).." ("..identity.phone..")"

          CLIMBclient.notify(dest_src,{lang.phone.sms.notify({from, msg})})
...
-- cbreturn true on success
function CLIMB.sendSMSPos(user_id, phone, x,y,z, cbr)
  local task = Task(cbr)
  ...
  CLIMB.getUserIdentity(user_id, function(identity)
    CLIMB.getUserByPhone(phone, function(dest_id)
      if identity and dest_id then
        ...
        if dest_src then
          local from = CLIMB.getPhoneDirectoryName(dest_id, identity.phone).." ("..identity.phone..")"
          CLIMBclient.notify(dest_src,{lang.phone.smspos.notify({from})}) -- notify
          -- add position for 5 minutes
          CLIMBclient.addBlip(dest_src,{x,y,z,162,37,from}, function(bid)
            SetTimeout(cfg.smspos_duration*1000,function()
              CLIMBclient.removeBlip(dest_src,{bid})
            end)
          end)
...
function CLIMB.getPhoneDirectory(user_id)
...
function CLIMB.getPhoneDirectoryName(user_id, phone)
  local directory = CLIMB.getPhoneDirectory(user_id)
...
function CLIMB.getPhoneSMS(user_id)
  local data = CLIMB.getUserTmpTable(user_id)
  if data then
...
local function ch_directory(player,choice)
  ...
  if user_id ~= nil then
    local phone_directory = CLIMB.getPhoneDirectory(user_id)
    ...
    local ch_add = function(player, choice) -- add to directory
      CLIMB.prompt(player,lang.phone.directory.add.prompt_number(),"",function(player,phone)
        CLIMB.prompt(player,lang.phone.directory.add.prompt_name(),"",function(player,name)
          ...
          if #name > 0 and #phone > 0 then
            ...
            CLIMBclient.notify(player, {lang.phone.directory.add.added()})
          else
    ...
    local ch_entry = function(player, choice) -- directory entry menu
      ...
      local ch_remove = function(player, choice) -- remove directory entry
        ...
        CLIMB.closeMenu(player) -- close entry menu (removed)
      end

      local ch_sendsms = function(player, choice) -- send sms to directory entry
        CLIMB.prompt(player,lang.phone.directory.sendsms.prompt({cfg.sms_size}),"",function(player,msg)
          msg = sanitizeString(msg,sanitizes.text[1],sanitizes.text[2])
          CLIMB.sendSMS(user_id, phone, msg, function(ok)
            if ok then
              CLIMBclient.notify(player,{lang.phone.directory.sendsms.sent({phone})})
            else
              CLIMBclient.notify(player,{lang.phone.directory.sendsms.not_sent({phone})})
            end
          end)
        end)
      end
      ...
      local ch_sendpos = function(player, choice) -- send current position to directory entry
        CLIMBclient.getPosition(player,{},function(x,y,z)
          CLIMB.sendSMSPos(user_id, phone, x,y,z,function(ok)
            if ok then
              CLIMBclient.notify(player,{lang.phone.directory.sendsms.sent({phone})})
            else
              CLIMBclient.notify(player,{lang.phone.directory.sendsms.not_sent({phone})})
    ...
    -- menu.onclose = function(player) CLIMB.openMenu(player, phone_menu) end
...
local function ch_sms(player, choice)
  local user_id = CLIMB.getUserId(player)
  if user_id ~= nil then
    local phone_sms = CLIMB.getPhoneSMS(user_id)
    ...
    for k,v in pairs(phone_sms) do
      local from = CLIMB.getPhoneDirectoryName(user_id, v[1]).." ("..v[1]..")"
      local phone = v[1]
...
-- nest menu
service_menu.onclose = function(player) CLIMB.openMenu(player, phone_menu) end

local function ch_service_alert(player,choice) -- alert a service
  local service = services[choice]
  if service then
    CLIMBclient.getPosition(player,{},function(x,y,z)
      CLIMB.prompt(player,lang.phone.service.prompt(),"",function(player, msg)
			  msg = sanitizeString(msg,sanitizes.text[1],sanitizes.text[2])
	  	  if msg ~= nil and msg ~= "" then
			    CLIMBclient.notify(player,{service.notify}) -- notify player
			    CLIMB.sendServiceAlert(player,choice,x,y,z,msg) -- send service alert (call request)
		    else
			    CLIMBclient.notify(player,{"Empty Message."})
	      end
      end)
    end)
  end
...
local function ch_service(player, choice)
  CLIMB.openMenu(player,service_menu)
end
...
-- nest menu
announce_menu.onclose = function(player) CLIMB.openMenu(player, phone_menu) end
...
local function ch_announce_alert(player,choice) -- alert a announce
  ...
  if announce and user_id ~= nil then
    if announce.permission == nil or CLIMB.hasPermission(user_id,announce.permission) then
      CLIMB.prompt(player,lang.phone.announce.prompt(),"",function(player, msg)
        msg = sanitizeString(msg,sanitizes.text[1],sanitizes.text[2])
        if string.len(msg) > 10 and string.len(msg) < 1000 then
          if announce.price <= 0 or CLIMB.tryPayment(user_id, announce.price) then -- try to pay the announce
            CLIMBclient.notify(player, {lang.money.paid({announce.price})})
            ...
            for k,v in pairs(users) do
              CLIMBclient.announce(v,{announce.image,msg})
            end
          else
      ...
      CLIMBclient.notify(player, {lang.common.not_allowed()})
    end
  end
end
...
local function ch_announce(player, choice)
  CLIMB.openMenu(player,announce_menu)
end
...
SetTimeout(10000, function()
  CLIMB.buildMenu("phone", {}, function(menu)
