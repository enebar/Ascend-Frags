...
RegisterNetEvent("CLIMB:showWarningsOfUser")
AddEventHandler("CLIMB:showWarningsOfUser",function(CLIMBwarningstables)
	showWarningSystem = true
	CLIMBWarnings = CLIMBwarningstables
end)

RegisterNetEvent("CLIMB:recievedRefreshedWarningData")
AddEventHandler("CLIMB:recievedRefreshedWarningData",function(CLIMBwarningstables)
	CLIMBWarnings = CLIMBwarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("CLIMB:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			for warningID,warningTable in pairs(CLIMBWarnings) do
