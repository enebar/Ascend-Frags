...
--     if(user_id)then
--         if bool == true then
--         CLIMB.giveMoney({user_id, math.random(100000,350000)})
...
--     local player = CLIMB.getUserSource({user_id})
--     if(user_id)then
--         if CLIMB.tryGetInventoryItem({user_id,"bank_drill",1,false}) then
--             TriggerClientEvent('bankofenglandrobbery:boxuse1', source)
--         else
--             CLIMBclient.notify(player,{"You don't have ~d~1 Drill"})
--         end
--     end
-- end)
...
--         CLIMBclient.notify(player,{"~d~Cops can't rob banks."})
--     else
--         if #cops >= minCops then
--             if CLIMB.tryGetInventoryItem({user_id,"bank_phone",1,false}) then
--                 TriggerClientEvent('bankofenglandrobbery:phoneclient1', source)
--             else
--                 CLIMBclient.notify(player,{"~d~You don't have a hacking phone."})
--             end
--         else
--             CLIMBclient.notify(player,{"~d~Not enough cops online."})
--         end
--     end
-- end)
...
--         else
--             CLIMBclient.notify(player,{"You don't have ~d~1 Hacking Phone"})
--         end
--     end
-- end)
...
--     local player = CLIMB.getUserSource({user_id})
--     if(user_id)then
--         if CLIMB.hasPermission({user_id,"bank.doorbutton"}) then
--             TriggerClientEvent('bankofenglandrobbery:policecheckpermdoorSUCC', source)
--         else
--             CLIMBclient.notify(player,{"~d~You are not a policeman or a banker!"})
--         end
--     end
-- end)
...
--     local player = CLIMB.getUserSource({user_id})
--     if(user_id)then
--         if CLIMB.hasPermission({user_id,"bank.doorbuttontimed"}) then
...
--         if user_id ~= nil then
--           if CLIMB.hasPermission({user_id, "police.armoury"}) then
--             TriggerClientEvent("CLIMB:setBankRobberyBlip", v)
--           end
--         end
--     end
-- end)
