          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                CLIMBcfgdrugsServer.DiamondGather()
                Action = false
              else
                CLIMB.notify({"~d~How the fuck are you supposed to gather sitting in that thing?"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              CLIMBcfgdrugsServer.DiamondCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  CLIMBcfgdrugsServer.DiamondDoneProcessing()
                  CLIMB.notify({"~w~Processed~g~ Diamond~w~ 1."})
                  Action = false

                end
                if not canProcess then
                  CLIMB.notify({"~d~You do not have the required license."})
                  else
                    if not ketAmount then
                      CLIMB.notify({"~d~You do not have enough Diamond Crystals."})
                    end
                end
              end)
...
function sellingDiamondUnits()
     CLIMBcfgdrugsServer.SellDiamondJob({tonumber(1)})
    return false
end
