...
function tCLIMB.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tCLIMB.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
...
function tCLIMB.setHandcuffed(flag)
  if handcuffed ~= flag then
    tCLIMB.toggleHandcuff()
  end
end

function tCLIMB.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tCLIMB.putInNearestVehicleAsPassenger(radius)
...
function tCLIMB.putInNetVehicleAsPassenger(net_veh)
...
function tCLIMB.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tCLIMB.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tCLIMB.jail(x,y,z,radius)
  tCLIMB.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tCLIMB.unjail()
  jail = nil
end

function tCLIMB.isJailed()
  return jail ~= nil
end
