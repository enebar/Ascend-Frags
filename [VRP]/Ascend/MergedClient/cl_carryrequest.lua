...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("CLIMB:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("CLIMBEXTRAS:StartCarry")
AddEventHandler("CLIMBEXTRAS:StartCarry", function(targetSrc)
...
RegisterNetEvent("CLIMBEXTRAS:CarryTargetAsk")
AddEventHandler("CLIMBEXTRAS:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("CLIMBEXTRAS:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("CLIMBEXTRAS:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
