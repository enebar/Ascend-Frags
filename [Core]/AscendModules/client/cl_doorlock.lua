...
RegisterNetEvent('CLIMB_doors:load')
AddEventHandler('CLIMB_doors:load', function(list)
    doors = list
end)
RegisterNetEvent('CLIMB_doors:statusSend')
AddEventHandler('CLIMB_doors:statusSend', function(id,status)
        ...
        if v.close ~= nil then
            ...
            if door1 < newrange then
                if door1 < GetDistanceBetweenCoords(x,y,z,doors[v.close].coords[1], doors[v.close].coords[2], doors[v.close].coords[3],true) then
                    TriggerServerEvent("CLIMB_doors:status", k, not v.locked)
                else
                    TriggerServerEvent("CLIMB_doors:status", v.close, not doors[v.close].locked)
                end
                break
            end
        else
