...
items["money"] = {"Money","Packed money.",function(args)
  ...
  choices["Unpack"] = {function(player,choice,mod)
    ...
    -- if user_id ~= nil then
    --   local amount = CLIMB.getInventoryItemAmount(user_id, idname)
    --   CLIMB.prompt(player, "How much to unpack ? (max "..amount..")", "", function(player,ramount)
    --     ramount = parseInt(ramount)
    --     if CLIMB.tryGetInventoryItem(user_id, idname, ramount, true) then -- unpack the money
    --       CLIMB.giveMoney(user_id, ramount)
...
items["money_binder"] = {"Money binder","Used to bind £1000 of money.",function(args)
  ...
  choices["Bind money"] = {function(player,choice,mod) -- bind the money
    ...
    --   if money >= 1000 then
    --     if CLIMB.tryGetInventoryItem(user_id, idname, 1, true) and CLIMB.tryPayment(user_id,1000) then
    --       CLIMB.giveInventoryItem(user_id, "money", 1000, true)
    --       CLIMB.closeMenu(player)
    --     end
    --   else
    --     CLIMBclient.notify(player,{CLIMB.lang.money.not_enough()})
    --   end
    -- end
  end}
...
local wbody_choices = function(args)
  ...
  choices["Equip"] = {function(player,choice)
    local user_id = CLIMB.getUserId(player)
    if user_id ~= nil then
      if CLIMB.tryGetInventoryItem(user_id, fullidname, 1, true) then -- give weapon body
        local weapons = {}
        weapons[args[2]] = {ammo = 0}
        CLIMBclient.giveWeapons(player, {weapons})
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    local user_id = CLIMB.getUserId(player)
    if user_id ~= nil then
      local amount = CLIMB.getInventoryItemAmount(user_id, fullidname)
      TriggerClientEvent('checkAmmo', player)
      CLIMB.prompt(player, "Amount to load ? (max "..amount..")", "", function(player,ramount)
        ramount = parseInt(ramount)

        CLIMBclient.getWeapons(player, {}, function(uweapons)
            for i,v in pairs(CLIMBAmmoTypes[ammotype]) do
               if uweapons[v] ~= nil then -- check if the weapon is equiped
                      if CLIMB.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- give weapon ammo
                        local weapons = {}
                        weapons[v] = {ammo = ramount}
                        CLIMBclient.giveWeapons(player, {weapons,false})
                        CLIMB.closeMenu(player)
...
items["heavyarmour"] = {"Heavy Armour Plate","Armour",function(args)
  local choices = {}
  choices['Equip'] = {function(player,choice)
    local PermID = CLIMB.getUserId(player)
    local Ped = GetPlayerPed(player)
    local PedArmour = GetPedArmour(Ped)
    if Cooldown[player] then
      return CLIMBclient.notify(player,{"~d~Server is still proccesing your request."})
    else
      if PermID ~= nil then
        if PedArmour > 95 then
          CLIMBclient.notify(player,{"~d~You already have armour."})
        else
          if CLIMB.hasPermission(PermID, "rebel.guns") then
            if CLIMB.tryGetInventoryItem(PermID, "heavyarmour", 1, false) then
              Cooldown[player] = true;
              CLIMBclient.playAnim(player, {false, armour_seq, false}) -- anim
              Wait(2500)
              CLIMBclient.setArmour(player,{100})
              CLIMBclient.notify(player,{"~g~Applied Armour Plate"})
              Cooldown[player] = false;
            end
          else
            CLIMBclient.notify(player,{"~d~Missing Rebel License."})
          end
        end
      end
    end
...
local function play_morphine(player)
  ...
  CLIMBclient.playAnim(player,{true,morphine_seq,false})
end
...
choices['Equip'] = {function(player,choice)
  local PermID = CLIMB.getUserId(player)
  if PermID ~= nil then 
  if CLIMB.tryGetInventoryItem(PermID, "morphine", 1, false) then 
...
for i,v in pairs(CLIMBAmmoTypes) do
  -- print(i)
