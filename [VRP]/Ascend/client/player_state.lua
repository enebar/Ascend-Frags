...
function tCLIMB.giveWeapons(weapons,clear_before)
  ...
  for k,weapon in pairs(weapons) do
    ...
    tCLIMB.allowWeapon(k)
    GiveWeaponToPed(player, hash, ammo, false)
  end
end

function tCLIMB.giveWeaponAmmo(hash, amount)
...
function tCLIMB.getDrawables(c)
  local d,e=a(c)
  if d then 
...
function tCLIMB.getDrawableTextures(c,f)
  local d,e=a(c)
  if d then 
...
function tCLIMB.setCustomization(h,j,k)
  if h then 
      local g=PlayerPedId()
      local l=nil;
...
function tCLIMB.spawnAnim(a)
  if a ~= nil then
    ...
    NewLoadSceneStop()
    tCLIMB.checkCustomization()
    TriggerServerEvent('CLIMB:changeHairstyle')
    TriggerServerEvent('CLIMB:getPlayerTattoos')
    TriggerEvent("CLIMB:playGTAIntro")
    DoScreenFadeIn(1000)
    ShutdownLoadingScreen()
    ShutdownLoadingScreenNui()
    if not tCLIMB.isDevMode() then
    ...
    print("[CLIMB] cachedUserData.health", userdata.health)
    print("[CLIMB] cachedUserData.armour", userdata.armour)
    SetEntityHealth(PlayerPedId(), userdata.health or 200)
    tCLIMB.setArmour(userdata.armour)
...
AddEventHandler("CLIMB:playGTAIntro",function()
  if not tCLIMB.isDevMode() then
