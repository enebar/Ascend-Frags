...
function CLIMB.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  CLIMBclient.openMenuData(source,{menudata})
end

-- force close player menu
function CLIMB.closeMenu(source)
  CLIMBclient.closeMenu(source,{})
end
...
function CLIMB.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  CLIMBclient.prompt(source,{title,default_text})
end
...
function CLIMB.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  CLIMBclient.request(source,{id,text,time}) -- send request to client
...
function CLIMB.registerMenuBuilder(name, builder)
...
function CLIMB.buildMenu(name, data, cbr)
...
function CLIMB.openMainMenu(source)
  CLIMB.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    CLIMB.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tCLIMB.closeMenu(id)
...
function tCLIMB.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tCLIMB.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tCLIMB.requestResult(id,ok)
...
function tCLIMB.openMainMenu()
  CLIMB.openMainMenu(source)
end
...
function CLIMB.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for CLIMB.addStaticMenuChoices calls
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        local function smenu_enter()
          local user_id = CLIMB.getUserId(source)
          if user_id ~= nil and CLIMB.hasPermissions(user_id,smenu.permissions or {}) then
        ...
        CLIMBclient.addBlip(source,{x,y,z,smenu.blipid,smenu.blipcolor,smenu.title})
        CLIMBclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,226,0,125,150})

        CLIMB.setArea(source,"CLIMB:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("CLIMB:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    CLIMBclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
