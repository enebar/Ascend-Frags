...
tCLIMB.hasPoliceJob = function(bool)
    police = bool
end

tCLIMB.PanicCL = function(player, s1, s2, name)
    if police then
...
tCLIMB.BlipCL = function(tx, ty, tz)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                CLIMBserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                CLIMBserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                CLIMBserver.PanicSV({player,  "No street found", ""})
                CLIMBserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
