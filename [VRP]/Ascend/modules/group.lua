...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        CLIMB.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = CLIMB.getUserId(source)
                    if user_id ~= nil and CLIMB.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                CLIMBclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                CLIMBclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                CLIMB.setArea(source, "CLIMB:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
