          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                CLIMBcfgdrugsServer.LSDGather()
                Action = false
              else
                CLIMB.notify({"~d~You cant gather Lysergic Acid while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              CLIMBcfgdrugsServer.CanProcessLSD({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  CLIMBcfgdrugsServer.ProcessLSD()

                  CLIMB.notify({"~w~Processed~g~ LSD~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  CLIMB.notify({"~d~You do not have the required license."})
                    ...
                    if not ketAmount then
                      CLIMB.notify({"~d~You do not have enough Lysergic Acid."})
                    end
                end
              end)
...
function sellingLSDUnits()
    CLIMBcfgdrugsServer.SellLSD({tonumber(1)})
    return false
end
