    ...
    RageUI.IsVisible(RMenu:Get('MUSICMENU','main'),true,false,true,function()
        if not a.active then 
            ...
            RageUI.ButtonWithStyle("Help","Assistance message",{},true,function(k,l,m)
                if m then 
                    TriggerEvent("CLIMB:showNotification",{text="The Video ID is the ID at the end of the YouTube URL after =",height="200px",width="auto",colour="#FFF",background="#32CD32",pos="bottom-right",icon="success"},5000)
                end 
            end)
        end 
    end,function()
            ...
            for o,p in pairs(i)do 
                if i[o]~=nil then 
                    ...
                    RageUI.ButtonWithStyle("ID: "..p[3].." - Started: "..p[5].."","Name: "..p[4].." Distance Away: "..distance.." metres",{RightLabel="→→→"},true,function(k,l,m)
                        if m then 
                            TriggerServerEvent("CLIMB:adminStopSong",p[1])
                            table.remove(i,o)
                        end 
                    end)
                end 
...
RegisterNetEvent("CLIMB:playDjSong")
AddEventHandler("CLIMB:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("CLIMB:requestCurrentProgress")
AddEventHandler("CLIMB:requestCurrentProgress",function(s,coords)
    if not g then 
...
RegisterNetEvent("CLIMB:toggleDjMenu")
AddEventHandler("CLIMB:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('MUSICMENU','main'),true)
    h=true 
end)
RegisterNetEvent("CLIMB:toggleDjAdminMenu")
AddEventHandler("CLIMB:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('MUSICMENU','admin'),true)
    i=x 
end)
RegisterNetEvent("CLIMB:finaliseSong")
AddEventHandler("CLIMB:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("CLIMB:playDjSongServer",y,a.coords)
    end 
end)
RegisterNetEvent("CLIMB:updateDjVolume")
AddEventHandler("CLIMB:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        SendNUIMessage({type='djVolume',volume=z})
    end 
end)
RegisterNetEvent("CLIMB:stopSong")
AddEventHandler("CLIMB:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("CLIMB:skipDj")
AddEventHandler("CLIMB:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("CLIMB:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
...
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("CLIMB:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("CLIMB:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('CLIMB:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
...
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("CLIMB:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("CLIMB:returnProgress")
AddEventHandler("CLIMB:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
