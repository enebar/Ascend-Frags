    ...
    if #(playerCoords - boxCoords) < 2.0 then
        if not used then
                ...
                if lootrandom == 1 then -- [Legendary]
                    -- [Legendary]
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_SCORPIANBLUE', 1, true})
                    CLIMB.giveInventoryItem({user_id, '9mm Bullets', 250, true})

                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_CHERRYMOSIN', 1, true})
                    CLIMB.giveInventoryItem({user_id, '7,62 Bullets', 250, true})
                    CLIMBclient.notify(source,{'Received ~g~£1,000,000 Cash.'})
                    CLIMB.giveMoney({user_id,1000000})
                ...
                elseif lootrandom == 2 then 
                    -- [Epic]
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_NERFMOSIN', 1, true})
                    CLIMB.giveInventoryItem({user_id, '7.62 Bullets', 250, true})

                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_GOLDAK', 1, true})
                    CLIMBclient.notify(source,{'Received ~g~£700,000 Cash.'})
                    CLIMB.giveMoney({user_id,7000000})
                    TriggerClientEvent('chat:addMessage', -1, {
                ...
                elseif lootrandom == 3 then 
                    -- [Uncommon]
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_VESPER', 1, true})
                    CLIMB.giveInventoryItem({user_id, '9mm Bullets', 250, true})

                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_USPSKILLCONFIRMED', 1, true})
                    CLIMB.giveInventoryItem({user_id, '9mm Bullets', 50, true})
                    CLIMBclient.notify(source,{'Received ~g~£450,000 Cash.'})
                    CLIMB.giveMoney({user_id,450000})
                    TriggerClientEvent('chat:addMessage', -1, {
                ...
                elseif lootrandom == 4 then 
                    -- [Common]
                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_BROOM', 1, true})

                    CLIMB.giveInventoryItem({user_id, "wbody|" .. 'WEAPON_UMP45', 1, true})
                    CLIMB.giveInventoryItem({user_id, '9mm Bullets', 250, true})
                    CLIMBclient.notify(source,{'Received ~g~£200,00 Cash.'})
                    CLIMB.giveMoney({user_id,200000})
                    TriggerClientEvent('chat:addMessage', -1, {
...
RegisterServerEvent('updateRigLoot', function(source, item, amount)
    ...
    if #currentLoot == 0 then
        if not used then
            ...
            TriggerClientEvent('CLIMB:SendSecondaryInventoryData', source, currentLoot, CLIMB.computeItemsWeight({currentLoot}), 30)
end) 
