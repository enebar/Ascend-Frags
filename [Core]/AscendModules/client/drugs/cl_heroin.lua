          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                CLIMBcfgdrugsServer.HeroinGather ()
                Action = false
              else
                CLIMB.notify({"~d~You cant gather Heroin while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              CLIMBcfgdrugsServer.CanProcessHeroin({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  CLIMBcfgdrugsServer.ProcessHeroin()

                  CLIMB.notify({"~w~Processed~g~ Heroin~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  CLIMB.notify({"~d~You do not have the required license."})
                    ...
                    if not ketAmount then
                      CLIMB.notify({"~d~You do not have enough Opium."})
                    end
                end
              end)
...
function sellingHeroinUnits()
    CLIMBcfgdrugsServer.SellHeroin({tonumber(1)})
    return false
end
