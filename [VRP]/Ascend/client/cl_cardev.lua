                ...
                RageUI.Button("Cycle through seats" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if IsPedInAnyVehicle(p) then
                            ...
                            tCLIMB.notify("~d~Not in a vehicle.")
                        end
                    end
                end)
                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~d~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("CLIMB:setCarDev",b)
                        end
                    end)
                else
...
RegisterCommand('cardev', function()
    TriggerServerEvent('CLIMB:openCarDev')
end)

function tCLIMB.openCarDev()
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('CLIMB:takeCarScreenshot')
    Wait(5000)
...
local A={
    ...
    {name="fPercentSubmCLIMBed",type="float"},
...
local function a1()
    ...
    TriggerServerEvent('CLIMB:getCarDevDebug',{a2})
end
