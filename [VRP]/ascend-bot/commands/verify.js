...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.ghmattimysql.execute("SELECT * FROM `climb_verification` WHERE code = ?", [params[0]], (code) => {
        if (code.length > 0) {
           if (code[0].discord_id === null ){
            fivemexports.ghmattimysql.execute("UPDATE `climb_verification` SET discord_id = ?, verified = 1 WHERE code = ?", [message.author.id, params[0]], async (result) => {
                if (result) {
                    let embed = {
                        "title": "Verify",
                    ...
                    await message.member.addRole("989273243414298778").then().catch(console.error);
                }
            });
           }
           else{
